...
public class OrderController : Controller
{
    ...
    public async Task<IActionResult> Checkout(OrderModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            model.Cart = cart;
            var payment = await ProcessPayment(model); // await kullanıldı

            if (payment.Status == "success") // Artık bu çalışır
            {
    ...
    private async Task<Payment> ProcessPayment(OrderModel model)
    {
        ...
        options.ApiKey = "";//Doldur!
        options.SecretKey = "";//Doldur!
        ...
        buyer.Id = "BY789";
        buyer.Name = "Gökmen";
        buyer.Surname = "Soysal";
        buyer.GsmNumber = "+905350000000";
        buyer.Email = "gokmen@email.com";
        buyer.IdentityNumber = "74300864791";
        buyer.LastLoginDate = "2025-10-05 12:43:35";
        buyer.RegistrationDate = "2025-04-21 15:12:09";
        ...
        Payment payment = await Payment.Create(request, options); // await kullanıldı
        return payment;
    }
}